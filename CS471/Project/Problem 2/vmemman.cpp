...
void display(int pageSize, int noOfFrames, string algorithm, double pageFaultPercentage)
{
    ...
    ofstream fout;
    fout.open("output.txt",ios::app);
    fout<<pageSize<<"          "<<noOfFrames<<"              "<<algorithm<<"                "<<pageFaultPercentage<<"%"<<endl;
    fout.close();
}
...
void FIFO(int pageSize, int noOfFrames)
{
    ...
    //cout<<"\nnumber of pages: "<<noOfPages<<endl;//do we need this?
    ...
    while(PRQ.empty()==false)
    {
        ...
        else//if(fault==true)
        {
            ...
            if(fault==true)
            {
                ...
                loop=true;
                for(it=FRV.end()-1;loop==true;it--)//from the end of FRV to the current page fault
                {
                    for(int i=0; i < noOfFrames; i++)
                    {
                        if(*it==MM[i])//compare each frame to the current page reference

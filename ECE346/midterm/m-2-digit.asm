...
COUNT   	FCB	$00	;one's place
COUNT2		FCB	$00	;ten's place
...
COMP	LDAB	0, X		;b=x
	...
	;if(down==1) dec? works without this
	INX			;x++
	INY			;y++
...
STATE1			LDAB	#1
	...
	;if(count==0 && DOWN==1) how to do logical and?
	;add the following to the top of the display function - if(pause==1) bra pause_loop

;if(state=0)increment count.......goes on the top of display


CHECK_RESET_DOWN	DEC	COUNT
CHECK_RESET_DOWN2	LDAA 	0, Y		;a=y
			STAA	PORTB		;portb=a
		 	LDAB	#0 	;if(count==0) resetd count=16
			CMPB	COUNT
			BEQ	RESET_DOWN
			BNE	OC3END

CHECK_RESET_UP		INC	COUNT
CHECK_RESET_UP2		LDAA 	0, Y		;a=y
			STAA	PORTB		;portb=a
			LDAB	#16	;if(count==16) reset
			CMPB	COUNT		;
			BEQ	RESET_UP
			BNE	OC3END
...
RESET_UP		LDAB	#0	;count=0
			...
			INC	COUNT2
			BRA	OC3END
			;JSR	INPUT	;go back to input loop
